...
$id_staffs_edit = mysqli_real_escape_string($koneksi,$_POST['id_staffs_edit']);
$nama_staffs = mysqli_real_escape_string($koneksi,$_POST['nama_staffs']);
...
if($name_image1 != "/image/staffs/0.jpg" && $name_image1 != "") {
  ...
  $name_image1 = $updated_by.'-'.$nama_staffs.'.'.$imageFileType;
  ...
  $name_image1 = $fotoLama;
    ...
    $stmt1 = $koneksi->prepare("UPDATE `wb_staffs` set nama_staffs = ?, photos_staffs = ?,
    updated_at = ?, updated_by = ? where id_staffs=?");
    $stmt1->bind_param("sssss", $nama_staffs, $name_image1, $tanggal_now, $updated_by, $id_staffs_edit);
